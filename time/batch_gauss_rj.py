...
for i in data_files:
    ...
    axs[0].plot(x, multi_gaussian(x, *popt), color='r')
...
depo = np.array(depo)
time = np.array(time)
data = np.round(np.concatenate((temp[:,None], depo[:,None],time[:,None], data_dict), axis=1),8)
...
p = pd.DataFrame(data, columns=['temp','dep', 'time',
...
# # #save to text file
p.to_csv('gaussian_fits_time.csv', index=False)
